...
class FreeScale(object):
    ...
    def __call__(self, img, mask):
        assert img.size == mask.size
        return img.resize(self.size, Image.BILINEAR), mask.resize(
...
class Resize(object):
    ...
    def __call__(self, img, mask):
        return F.resize(img, self.size, Image.BILINEAR), F.resize(
...
class RandomSized(object):
    ...
    def __call__(self, img, mask):
        ...
        img, mask = img.resize(
            (w, h), Image.BILINEAR), mask.resize(
            (w, h), Image.NEAREST)
...
class SlidingCropOld(object):
    ...
    def __call__(self, img, mask):
        ...
        if long_size > self.crop_size:
            ...
            h_step_num = int(
                math.ceil(
                    (h - self.crop_size) / float(stride))) + 1
            w_step_num = int(
                math.ceil(
                    (w - self.crop_size) / float(stride))) + 1
    ...
    def __call__(self, img, mask):
        ...
        if long_size > self.crop_size:
            ...
            if h > self.crop_size:
                h_step_num = int(
                    math.ceil(
                        (h - self.crop_size) / float(stride_tmp))) + 1
                assert h_step_num > 1
            ...
            if w > self.crop_size:
                w_step_num = int(
                    math.ceil(
                        (w - self.crop_size) / float(stride_tmp))) + 1
                assert w_step_num > 1
...
class SlidingCropImageOnly(object):
    # stride_reate supports different strides in w and h, [stride_h, stride_w]
    ...
    def __call__(self, img):
        ...
        if long_size > self.crop_size:
            ...
            if h > self.crop_size:
                h_step_num = int(
                    math.ceil(
                        (h - self.crop_size) / float(stride_h))) + 1
                assert h_step_num > 1
                stride_yy = (h - self.crop_size) / (h_step_num - 1.)
            else:
                h_step_num = 1
            ...
            if w > self.crop_size:
                w_step_num = int(
                    math.ceil(
                        (w - self.crop_size) / float(stride_w))) + 1
                assert w_step_num > 1
                stride_xx = (w - self.crop_size) / (w_step_num - 1.)
            else:
                w_step_num = 1
...
class CorrResize(object):
    ...
    def __call__(self, img1, img2, pts1, pts2):
        asd = 0
        F.resize(
            img, self.size, Image.BILINEAR), F.resize(
            mask, self.size, Image.NEAREST)
